...
function safeState(payload){ return payload && payload.state ? payload.state : {}; }
...
export default async function handler(req, res){
  if(req.method === 'GET') return json(res, 200, { ok:true, service:'Ritme Student API', provider:AI_PROVIDER, model: AI_PROVIDER === 'groq' ? GROQ_MODEL : GEMINI_MODEL });
  ...
  try{
    ...
    if(action === 'aiCoach'){
      ...
      const answer = await callAI(`Data user:
${buildContext(state)}
